...
<html>
  ...
  <script>
    function showTable2()
    {
      ...
      xmlhttp.open("GET","showScheme.php?txtFeeId2="+document.getElementById("txtFeeId2").value,false);
      xmlhttp.send(null);
                             ...
                             <h4>Student Name: <?php echo $row['name'] ?></h4>
                                 <div class="col-md-12" style="margin-top: 6%">
                                    <table id="datatable1" class="table table-bordered table-striped">
                                      ...
                                      <?php endwhile; ?> <!-- mandatory fees -->  
                                      <?php 
                                      $query="select * from tblfee where tblFeeMandatory='N' and tblFeeFlag=1 group by tblFeeId";
                                      $result=mysqli_query($con, $query);
                                      while($row=mysqli_fetch_array($result)):
                                      ?>
                                        <tr>
                                          <td hidden><input type="hidden" name="txtFeeId2[]" id="txtFeeId2" value="<?php echo $row['tblFeeId'] ?>" /><?php echo $row['tblFeeId'] ?></td>
                                          <td><input type="checkbox" name="c1" id="c1" onclick="showTable2()"/></td>
                                          <td><?php echo $row['tblFeeCode'] ?></td>
                                          <td><?php echo $row['tblFeeName'] ?></td>
                                          <td>
                                          <?php
