...
<head>
  ...
  <![endif]-->
  <script>
  function changetblStudSect()
  {
    var lvl = document.getElementById("selSection").value;
    var xmlhttp =  new XMLHttpRequest();
    xmlhttp.open("GET","changeTblStudSect.php?selSection="+document.getElementById("selSection").value,false);
    xmlhttp.send(null);
          ...
          <div class="box box-primary">
            ...
            <form role="form">
              ...
              <div class="form-group">
                <label>Section</label>
                <select class="form-control select2" style="width: 50%;" name="selSection" id="selSection" onchange="changetblStudSect()">
                  <option selected="selected" disabled>--Select Section--</option>
                  <?php
                    $query="select * from tblsection where tblSectionFlag=1";
                    $result=mysqli_query($con, $query);
                    while($row=mysqli_fetch_array($result)):
                      $lvl=$row['tblSection_tblLevelId'];
                      $query1="select * from tbllevel where tblLevelId='$lvl' and tblLevelFlag=1";
                      $result1=mysqli_query($con, $query1);
                      $row1=mysqli_fetch_array($result1);
                      $lvlname=$row1['tblLevelName'];
                  ?>
                  <option value="<?php echo $row['tblSectionId'] ?>"><?php echo $lvlname.": ".$row['tblSectionName'] ?></option>
                  <?php endwhile; ?>
                </select>
              </div>
              </div>
