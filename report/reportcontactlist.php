...
$query = mysqli_query($con, "Select a.tblStudentId, concat(ti.tblStudInfoLname, ', ', ti.tblStudInfoFname, ' ', ti.tblStudInfoMname) as studentname, concat(p.tblParentLname, ', ', p.tblParentFname, ' ', p.tblParentMname) as parentname, concat (p.tblParentAddBldg, ' ', p.tblParentAddSt, ' ', p.tblParentAddBrgy, ' ', p.tblParentAddCity, ' ', p.tblParentAddCountry)  as address, p.tblParentCpNo, p.tblParentWorkNo, p.tblParentEmail, concat(fc.tblFacultyLname, ', ', fc.tblFacultyFname, ' ', fc.tblFacultyMname) as facultyname, st.tblSectionName, lv.tblLevelName FROM tblparent p, tblstudent a, tblparentstudent pt, tblstudentinfo ti, tblsection st, tblfaculty fc, tblLevel lv WHERE p.tblParentId=pt.tblParStud_tblParentId and a.tblStudentId=pt.tblParStud_tblStudentId and a.tblStudentId=ti.tblStudInfo_tblStudentId and p.tblParentFlag=1 and a.tblStudentFlag=1 and a.tblStudent_tblSectionId = st.tblSectionId and st.tblSection_tblFacultyId = fc.tblFacultyId and st.tblSection_tblLevelId=lv.tblLevelId and st.tblSectionId = 1 group by p.tblParentId");
